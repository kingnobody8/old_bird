...
//#include "engine/component/part.h" //TODO I am adding this here for the templated function 'FindPart' try to find another way to do this without including part
...
namespace Engine
{
	...
	namespace Object
	{
		...
		protected:
			std::string					m_szName;
			CGroup*						m_pParent;
			std::vector<Part::IPart*>	m_vParts;
			Math::CMatrix2D				m_cMatLocal;
			float						m_fZed;
			Util::Flag8					m_bVisible;
		public:
		...
		public:
			...
			virtual void AttachPart(Part::IPart* const pPart);		//Attaches a part script to the 'this' object, CALL INIT YOURSELF
			virtual bool DetachPart(Part::IPart* const pPart);		//Calls the destroy function for the script, then detaches it from the object, return true for success

			virtual void LoadJson(const Json::JSON& json);
			//virtual const Json::JSON SaveJson(void) const; //TODO

			//Gets
			inline const std::string&				GetName(void) const { return this->m_szName; }
			inline CGroup*							GetParent(void) const { return this->m_pParent; }
			inline const std::vector<Part::IPart*>&	GetParts(void) const { return this->m_vParts; }
			inline const Math::CMatrix2D&			GetLocalMatrix(void) const { return this->m_cMatLocal; }
			...
			//Calcs
			const Math::CMatrix2D					CalcWorldMatrix(void) const;
			...
			const Math::AABB						CalcAabb(void) const;
			...
			void									SetLocalMatrix(const Math::CMatrix2D& mat);
			void									SetWorldMatrix(const Math::CMatrix2D& mat);
			...
			type*									FindPart() const;
			Part::IPart*							FindPart(const std::string& szType) const;
			Part::IPart*							FindPart(const Part::PartTypeKey& key) const;
		...
		const Part::PartList			GetPartList(const Object::CObject* const pObj, const bool& reverse_priority = false);
		void							TraverseAncestors(const CObject* const that, std::function<void(CObject*const)> func, const EDirection eDirection = EDirection::UP);//Traverses through each parent either from 'that' to the root or from the root to 'that'
